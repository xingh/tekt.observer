...
- Roadmap item: RM-019 (`docs/roadmap.md`). Depends on RM-016 (crawl4ai integration) Phase 1; complements RM-018's `/explore` phase.
...
- [ ] Document the probe step in `docs/contributing/adding-sources.md` and reference it from the `set-up` skill
...
No implementation has started. Phase 1 is blocked on RM-016 (crawl4ai `crawl4ai_browser` mode) landing first, since the probe output targets that mode's config shape. The `browser-use skill install` step is a user-local machine action, not a repo change — if adopted, document it in `docs/machine_setup.md`.
